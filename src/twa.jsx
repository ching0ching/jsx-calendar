...
export default function DigitalEventCalendar() {
  console.log('DigitalEventCalendar component loaded');
...
// import / export
  ...
  function monthSelectOptions() {
    const opts = [];
    for (let y = 2025; y <= 2026; y++) for (let m = 1; m <= 12; m++) opts.push({ value: `${y}-${String(m).padStart(2, '0')}`, label: `${new Date(y, m - 1).toLocaleString([], { month: 'long' })} ${y}` });
    return opts;
  }
      ...
      <header className={`max-w-5xl mx-auto mb-4 sm:mb-6 p-3 sm:p-4 rounded-lg sm:rounded-xl shadow-lg border ${isDark ? 'border-gray-700' : 'border-gray-200'} flex items-stretch justify-between gap-3`} style={{ backgroundColor: COLORS.green.dark }}>
        <div className="flex items-center gap-2 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-lg sm:text-2xl font-bold leading-snug whitespace-nowrap" style={{ color: '#f7d774', textShadow: '0 1px 0 rgba(0,0,0,0.15)' }}>Event Calendar</h1>
            <p className="text-xs sm:text-sm leading-tight whitespace-nowrap" style={{ color: '#ffeeba' }}>Tayabas Western Academy</p>
          </div>
        </div>
        ...
        <aside className={`fixed lg:static lg:col-span-1 p-3 sm:p-4 rounded-lg sm:rounded-xl shadow-lg border ${isDark ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-200'} ${showSidebar ? 'right-0' : '-right-80 lg:right-auto'} top-0 bottom-0 w-80 lg:w-auto z-40 lg:z-auto overflow-y-auto transition-all duration-300 ease-in-out`}>
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2 flex-wrap mt-2">
              {Object.keys(TYPE_META).map(k => {
                ...
                return (
                  <div key={k} className="flex items-center gap-1 text-xs" style={{ backgroundColor: meta.color.bg, color: meta.color.text, padding: '4px 8px', borderRadius: '4px' }}>
                    <span>{meta.icon}</span>
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="p-2 rounded-lg" style={{ backgroundColor: isDark ? '#1e293b' : '#f0f4ff', border: `1px solid ${isDark ? '#475569' : '#c7d7ff'}` }}>
                <div className="text-2xl font-bold" style={{ color: isDark ? '#fff' : '#5b21b6' }}>{events.length}</div>
                <div className="text-xs" style={{ color: isDark ? '#9ca3af' : '#5b21b6' }}>Total Events</div>
              </div>
              <div className="p-2 rounded-lg" style={{ backgroundColor: isDark ? '#1e293b' : '#fef3c7', border: `1px solid ${isDark ? '#475569' : '#fbbf24'}` }}>
                <div className="text-2xl font-bold" style={{ color: isDark ? '#fbbf24' : '#b45309' }}>{events.filter(e => e.type === 'holiday').length}</div>
                <div className="text-xs" style={{ color: isDark ? '#fbbf24' : '#b45309' }}>Holidays</div>
              </div>
              <div className="p-2 rounded-lg" style={{ backgroundColor: isDark ? '#1e293b' : '#fce7f3', border: `1px solid ${isDark ? '#475569' : '#fbcfe8'}` }}>
                <div className="text-2xl font-bold" style={{ color: isDark ? '#f472b6' : '#be185d' }}>{events.filter(e => e.type === 'birthday').length}</div>
                <div className="text-xs" style={{ color: isDark ? '#f472b6' : '#be185d' }}>Birthdays</div>
              </div>
              <div className="p-2 rounded-lg" style={{ backgroundColor: isDark ? '#1e293b' : '#e0e7ff', border: `1px solid ${isDark ? '#475569' : '#a5b4fc'}` }}>
                <div className="text-2xl font-bold" style={{ color: isDark ? '#a5b4fc' : '#4f46e5' }}>{events.filter(e => e.type === 'exam').length}</div>
                <div className="text-xs" style={{ color: isDark ? '#a5b4fc' : '#4f46e5' }}>Exams</div>
              </div>
              <div className="p-2 rounded-lg" style={{ backgroundColor: isDark ? '#1e293b' : '#ffedd5', border: `1px solid ${isDark ? '#475569' : '#fb923c'}` }}>
                <div className="text-2xl font-bold" style={{ color: isDark ? '#fb923c' : '#d97706' }}>{events.filter(e => e.type === 'meeting').length}</div>
                <div className="text-xs" style={{ color: isDark ? '#fb923c' : '#d97706' }}>Meetings</div>
              </div>
              <div className="p-2 rounded-lg" style={{ backgroundColor: isDark ? '#1e293b' : '#dcfce7', border: `1px solid ${isDark ? '#475569' : '#86efac'}` }}>
                <div className="text-2xl font-bold" style={{ color: isDark ? '#4ade80' : '#15803d' }}>{events.filter(e => e.type === 'event').length}</div>
                <div className="text-xs" style={{ color: isDark ? '#4ade80' : '#15803d' }}>Other Events</div>
              </div>
            </div>
          </div>
        </aside>
